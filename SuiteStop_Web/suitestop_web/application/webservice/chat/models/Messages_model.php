...
class Messages_model extends CI_Model
{
     ...
     * @return array $return_arr returns response of query block.
     */
    public function get_user_details_for_send_notifi($user_id = '', $receiver_id = '', $space_id = '')
    {
        try {
            $result_arr = array();
            $this->db->from("message AS m");
            ...
            $this->db->join("space as sp","sp.iSpace_id = ".$space_id."", "left");
            $this->db->select("sp.vSpace_name AS space_name");
            $this->db->limit(1);
            ...
            if(isset($result_arr) && $result_arr != "") {
                for( $i= 0 ; $i< count($result_arr); $i++) {
                     $result_arr[$i]['sender_profile_image']=  ($result_arr[$i]['sender_profile_image'] != "") ?  $this->config->item("AWS_BASE_PATH_NAME").$this->config->item("AWS_BUCKET_NAME").'/'.$this->config->item("AWS_FOLDER_NAME").'/user_profile/'. $result_arr[$i]['s_users_id'].'/'. $result_arr[$i]['sender_profile_image'] : ""; 
    ...
    public function check_space_exist_or_not($input_params = array())
    {
        try {
            ...
            $this->db->where("s.iSpace_id", $input_params['space_id']);
            ...
            {
                $success = 1;
                $result_arr['status'] = "Yes";
            } else{
                $success = 1;
                $result_arr['status'] = "No";
            }
        } catch (Exception $e) {
            $params_arr['db_query'] = $this->db->last_query();
            $this->general->apiLogger($params_arr, $e);
