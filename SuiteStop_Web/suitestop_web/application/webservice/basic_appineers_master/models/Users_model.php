...
class Users_model extends CI_Model
{
    ...
    public function update_mobile_number($params_arr = array(), $where_arr = array())
    {
        try {
            ...
            if (isset($where_arr["usertype"]) && $where_arr["usertype"] != "") {
                $this->db->where("eUserType =", $where_arr["usertype"]);
            }
    ...
    public function create_user($params_arr = array())
    {
        try {
            ...
            $this->db->set("eUserType", $params_arr["usertype"]);
    ...
    {  
        try {
            ...
            $this->db->select("u.eLogStatus AS u_log_status_updated");
            $this->db->select("u.eUserType AS usertype");
            if (isset($insert_id) && $insert_id != "") {
                $this->db->where("u.iUserId =", $insert_id);
            }
            ...
            $result_arr[0]['isUpdated'] = empty($result_arr[0]['u_about_me'])? "0" : "1";
    ...
    public function get_user_login_details_v1($auth_token = '', $where_clause = '')
    {
        ...
        try {
            ...
            $this->db->select("u.eOneTimeTransaction AS eOneTimeTransaction");
            /*if (isset($where_arr["user_type"]) && $where_arr["user_type"] != "") {
                $this->db->where("eUserType =", $where_arr["user_type"]);
            }  */          
            $this->db->where("" . $where_clause . "", FALSE, FALSE);
            $this->db->limit(1);
            $result_obj = $this->db->get();
     ...
     * @return array $return_arr returns response of query block.
     */
    public function get_user_by_mobile_number($mobile_number = '', $usertype = '')
    {
        //pr($mobile_number);
        //pr($usertype); die;
        try {
            ...
            $this->db->select("u.vLastName AS u_last_name");
            $this->db->select("u.vUserName AS u_user_name");
            $this->db->select("u.eUserType As u_user_type");
            $this->db->select("u.vEmail AS u_email");
            $this->db->select("u.vMobileNo AS u_mobile_no");
            $this->db->select("u.eStatus AS u_status");
            $this->db->select("u.eModeratorFlag AS eModeratorFlag");
            $this->db->select("(concat(u.vFirstName,\"\",u.vLastName)) AS msg_user_name", FALSE);
            if ((isset($mobile_number) && $mobile_number != "") && (isset($usertype) && $usertype != "")) {
                $this->db->where("u.vMobileNo =", $mobile_number);
                $this->db->where("eUserType = ", $usertype);
            }
     ...
     * @return array $return_arr returns response of query block.
     */
    public function check_mobile_number($mobile_number = '', $reset_key = '', $usertype = '')
    {
        try {
            $message = "";
            $result_arr = array();
            ...
            if ((isset($mobile_number) && $mobile_number != "") && (isset($usertype) && $usertype != "")) {
                $this->db->where("u.vMobileNo =", $mobile_number);
                $this->db->where("u.eUserType =", $usertype);
            }
    ...
    { 
        try {
            ...
            if (isset($params_arr["aboutme"])) {
                $this->db->set("tAboutMe", $params_arr["aboutme"]);
            }
            if (isset($params_arr["latitude"])&& !empty($params_arr["latitude"])) {
                $this->db->set("dLatitude", $params_arr["latitude"]);
            }
            if (isset($params_arr["longitude"])&& !empty($params_arr["longitude"])) {
                $this->db->set("dLongitude", $params_arr["longitude"]);
            }
            $res = $this->db->update("users");
            //echo $this->db->last_query(); exist();
            $affected_rows = $this->db->affected_rows();
            $db_error = $this->db->error();
            if ($db_error['code']) {
                throw new Exception('Database error! Error Code [' . $db_error['code'] . '] Error: ' . $db_error['message']);
    ...
    public function get_updated_details($user_id = '')
    {
        try {
            ...
            } elseif(isset($result_arr[0]["u_user_id"]) && $result_arr[0]["u_user_id"] > 0) 
            {
    ...
    public function get_user_profile_details($user_id = '')
    {
        try {
            ...
            $this->db->select("u.vLastName AS u_last_name");
            $this->db->select("u.vEmail AS u_email");
            $this->db->select("u.tAboutMe AS u_aboutme");
            $this->db->select("u.vMobileNo AS u_mobile_no");
            $this->db->select("u.eUserType AS usertype");
            $this->db->select("u.vProfileImage AS u_profile_image");
            $this->db->select("u.vAccessToken AS u_access_token");
    ...
    /**
     * Used to execute database queries for about me API.
     *
     * @param array $params_arr params_arr array to process query block.
     *
     * @return array $return_arr returns response of query block.
     */
    public function about_me($params_arr = array(), $where_arr = array())
    {
        try {
            $message = "";
            $result_arr = array();
            ...
            $this->db->start_cache();
            if (isset($where_arr["user_id"]) && $where_arr["user_id"] != "") {
                $this->db->where("u.iUserId =", $where_arr["user_id"]);
            }
            ...
            //echo $this->db->last_query(); exit();
            $affected_rows = $this->db->affected_rows();
            $result_arr[0]['isUpdated'] = ($affected_rows == '0')? "0" : "1";
            $db_error = $this->db->error();
            if ($db_error['code']) {
                throw new Exception('Database error! Error Code [' . $db_error['code'] . '] Error: ' . $db_error['message']);
            }
