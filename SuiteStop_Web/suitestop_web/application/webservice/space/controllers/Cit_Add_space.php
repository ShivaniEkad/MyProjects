...
/**
 * Description of Add space Extended Controller
 *
 * @module Extended Add space
 *
 * @class Cit_Add_space.php
 *
 * @path application\webservice\space\controllers\Cit_Add_space.php
 *
 * @author Suresh Nakate
 *
 * @date 06.09.2021
 */
class Cit_Add_space extends Add_space
{
    /**
     ...
     */
    public function PrepareHelperMessage($input_params =array(),$value = array())
    {
        $this->db->select('nt.tMessage AS tMessage');
        $this->db->from('mod_push_notify_template as nt');
        $this->db->where('nt.vTemplateCode', 'Space_created');
            $notification_text=$this->db->get()->result_array();
            $notification_text=$notification_text[0]['tMessage'];
            $notification_text = str_replace("|sender_name|", ucfirst($input_params['get_add_space_details_for_notifi'][0]['sender_name']),$notification_text);
