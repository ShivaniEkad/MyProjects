    ...
    */
    public function get_user_profile_details($params_arr = array())
    {
        try {
            ...
            $this->db->select("u.vLastName AS u_last_name");
            $this->db->select("u.vProfileImage AS u_profile_image");
            $this->db->select("u.vMobileNo AS u_mobile");
            $block_status = "(select ub.eStatus from user_block AS ub where ub.iUser_id =". $params_arr['user_id']." && ub.iBlock_user_id =". $params_arr['other_user_id'].") as block_status";
            ...
            if(isset($result_arr) && $result_arr != "") {
                for( $i= 0 ; $i< count($result_arr); $i++) {
                     $result_arr[$i]['u_profile_image']=  ($result_arr[$i]['u_profile_image'] != "") ?  $this->config->item("AWS_BASE_PATH_NAME").$this->config->item("AWS_BUCKET_NAME").'/'.$this->config->item("AWS_FOLDER_NAME").'/user_profile/'. $result_arr[$i]['u_user_id'].'/'. $result_arr[$i]['u_profile_image'] : ""; 
