...
/**
 * Description of Accept request Extended Controller
 *
 * @module Extended Accept request
 *
 * @class Cit_Accept_request.php
 *
 * @path application\webservice\space\controllers\Cit_Accept_request.php
 *
 * @author Suresh Nakate
 *
 * @date 06.09.2021
 */
class Cit_Accept_request extends Accept_request
{
    /**
    ...
    public function PrepareHelperMessage($input_params=array())
    {
        ...
        $this->db->where('nt.vTemplateCode', 'Accepte_request');
            $notification_text=$this->db->get()->result_array();
            $notification_text=$notification_text[0]['tMessage'];
            $notification_text = str_replace("|space_name|", ucfirst($input_params['get_user_change_status_notification'][0]['space_name']),$notification_text);
