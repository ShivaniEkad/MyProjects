...
/**
 * Description of Space Management Controller
 *
 * @category admin
 *
 ...
 *
 * @class Space_management.php
 *
 * @path application\admin\masters\controllers\Space_management.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Space_management extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->module_name = "space_management";
        ...
        $this->dropdown_arr = array(
            "status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Active',
                        'val' => $this->lang->line('SPACE_MANAGEMENT_ACTIVE')
                    ),
                    array(
                        'id' => 'Inactive',
                        'val' => $this->lang->line('SPACE_MANAGEMENT_INACTIVE')
                    )
                )
            ),
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "space_management_list",
                    "space_management_view",
                    "space_management_add",
                    "space_management_update",
                    "space_management_delete",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("space_management", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "space_management");
            ...
            $status_label = array(
                'js_lang_label.SPACE_MANAGEMENT_ACTIVE',
                'js_lang_label.SPACE_MANAGEMENT_INACTIVE',
            );

            $list_config = $this->space_management_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "space_management");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->space_management_model->default_filters,
            );
            ...
            {
                $this->loadView("space_management_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->space_management_model->getListConfiguration();
        $form_config = $this->space_management_model->getFormConfiguration();
        $extra_cond = $this->space_management_model->extra_cond;
        $groupby_cond = $this->space_management_model->groupby_cond;
        $having_cond = $this->space_management_model->having_cond;
        $orderby_cond = $this->space_management_model->orderby_cond;
        ...
        //pr($data_config);die
        $data_recs = $this->space_management_model->getListingData($data_config);

        //print_r($data_recs); exit;
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_SPACE_MANAGEMENT_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("space_management_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("space_management", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->space_management_model->getListConfiguration();
        $form_config = $this->space_management_model->getFormConfiguration();
        $table_name = $this->space_management_model->table_name;
        $table_alias = $this->space_management_modeltable_alias;
        $primary_key = $this->space_management_model->primary_key;
        $extra_cond = $this->space_management_model->extra_cond;
        $groupby_cond = $this->space_management_model->groupby_cond;
        $having_cond = $this->space_management_model->having_cond;
        $orderby_cond = $this->space_management_model->orderby_cond;
        ...
        $db_recs = $this->space_management_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('SPACE_MANAGEMENT_SPACE_MANAGEMENT');
        $misc_info['filename'] = "space_management_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("space_management_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("space_management", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("space_management", $access_list, TRUE, TRUE);
                }
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "space_management"), $this->session->userdata('iAdminId'));
                $data_arr = $this->space_management_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "space_management", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "space_management"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "space_management");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->space_management_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->space_management_model, $phpfunc))
                            {
                                $tmpdata = $this->space_management_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->space_management_model, $fd_callback))
                                {
                                    $fd_status = $this->space_management_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("space_management_add");
                    }
                    else
                    {
                        $this->loadView("space_management_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("space_management_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("space_management_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("space_management", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            $space_name = $params_arr["space_name"];
            $address = $params_arr["address"];
            $city = $params_arr["city"];
            $state = $params_arr["state"];
            $zipcode = $params_arr["zipcode"];
            $stylist_required = $params_arr["stylist_required"];
            $space_details = $params_arr["space_details"];
            ...
            $save_data_arr["space_name"] = $data["vSpace_name"];
            $save_data_arr["address"] = $data["tAddress"];
            $save_data_arr["city"] = $data["vCity"];
            $save_data_arr["state"] = $data["vState"];
            $save_data_arr["zipcode"] = $data["iZipcode"];
            $save_data_arr["stylist_required"] = $data["iStylist_required"];
            $save_data_arr["space_details"] = $data["tSpace_details"];
            $save_data_arr["status"] = $data["eStatus"];
            $strCheck = 0;
            if($id > 0)
            {
            ...
            if ($mode == 'Add')
            {
                ...
                $data["vSpace_name"] = $space_name;
                $data["tAddress"] = $address;
                $data["vCity"] = $city;
                $data["vState"] = $state;
                $data["iZipcode"] = $zipcode;
                $data["iStylist_required"] = $stylist_required;
                $data["tSpace_details"] = $space_details;
                $data["eStatus"] = $status;

                $id = $this->space_management_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iSpace_id"] = $data["iSpace_id"] = $id;
                ...
                $track_cond = $this->db->protect("iSpace_id")." = ".$this->db->escape($id);
                $switch_combo = $this->space_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "space_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update' && false== $strCheck)
            {
                ...
                $res = $this->space_management_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iSpace_id"] = $data["iSpace_id"] = $id;
                ...
                $switch_combo = $this->space_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "space_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
             if(true==$strCheck)
            {
                $msg = "This space is already in used. So can not be Deleted or Inactivated";
            }else{
                $ret_arr['message'] = $msg; 
            }
            $ret_arr['message'] = $msg;
    ...
    public function inlineEditAction()
    {
     ...
     //  echo "str--".$strCheck; exit;
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")) && false==$strCheck)
        {
            ...
            $config_arr['list_config'] = $this->space_management_model->getListConfiguration();
            $config_arr['form_config'] = $this->space_management_model->getFormConfiguration();
            $config_arr['table_name'] = $this->space_management_model->table_name;
            $config_arr['table_alias'] = $this->space_management_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->space_management_model->table_alias.".".$this->space_management_model->primary_key;
        }
        else
        {
            $primary_field = $this->space_management_model->primary_key;
        }
        ...
        try
        {
            ...
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("space_management_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("space_management", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("iSpace_id")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("iSpace_id")." = ".$this->db->escape($primary_ids);
                    }
                    if(false==$strCheck)
                    {
                       $success = $this->space_management_model->delete($extra_cond, $search_alias, $search_join);  
                    }else{
                        $success = 1;
                    }
                    $main_data = $this->space_management_model->getData($main_cond, array());
                    $success = $this->space_management_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("space_management_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("space_management", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->space_management_model->getListConfiguration($post_name);
                    $form_config = $this->space_management_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->space_management_model->getData(intval($primary_ids));

  
                    $data_arr[$field_name] = $post_val;
                    if(false==$strCheck)
                    {
                      $success = $this->space_management_model->update($data_arr, intval($primary_ids)); 
                    }else{
                        $success = 1;
                    }
                    $success = $this->space_management_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->space_management_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    if(false==$strCheck)
                    {
                      $success = $this->space_management_model->update($data_arr, $primary_ids);
                    }else{
                        $success = 1;                       
                    }
                    //$success = $this->space_management_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
            ...
            if ($mode == "Update")
            {
                ...
                if ($form_config["type"] == "file")
                {
                    ...
                    $file_data[$unique_name]["primary_key"] = "iSpace_id";
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->space_management_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->space_management_model, $phpfunc))
                    {
                        $data_arr = $this->space_management_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->space_management_model->switchto_fields;
        $extra_cond = $this->space_management_model->extra_cond;
        ...
        $switch_arr = $this->space_management_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->space_management_model->getListConfiguration($alias_name);
    ...
    public function downloadListFile()
    {
        ...
        if (is_array($config_arr) && count($config_arr) > 0)
        {
            ...
            $data_arr = $this->space_management_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->space_management_model->getFormConfiguration($unique_name);
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->space_management_model->update($update_arr, intval($id));
                if (!$res)
                {
