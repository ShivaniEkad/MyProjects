...
/**
 * Description of Request Management Controller
 *
 * @category admin
 *
 ...
 *
 * @class Request_management.php
 *
 * @path application\admin\masters\controllers\Request_management.php
 *
 * @version 4.4
 *
 * @author CIT Dev Team
...
class Request_management extends Cit_Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->load->model('request_management_model');
        ...
        $this->dropdown_arr = array(
            "status" => array(
                ...
                "values" => array(
                    array(
                        'id' => 'Accept',
                        'val' => $this->lang->line('REQUEST_MANAGEMENT_ACCEPT')
                    ),
                    array(
                        'id' => 'Reject',
                        'val' => $this->lang->line('REQUEST_MANAGEMENT_REJECT')
                    ),
                    array(
                        'id' => 'Pending',
                        'val' => $this->lang->line('REQUEST_MANAGEMENT_PENDING')
                    )
                )
            ),
        );
    ...
    public function index()
    {
        ...
        try
        {
            ...
            {
                $access_list = array(
                    "request_management_list",
                    "request_management_view",
                    "request_management_add",
                    "request_management_update",
                    "request_management_delete",
                );
            ...
            else
            {
                ...
                list($list_access, $view_access, $add_access, $edit_access, $del_access, $expo_access, $print_access) = $this->filter->getModuleWiseAccess("request_management", $access_list, TRUE, TRUE);
            }
            if (!$list_access)
            {
                throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_VIEW_THIS_PAGE_C46_C46_C33'));
            }
            $enc_loc_module = $this->general->getMD5EncryptString("ListPrefer", "request_management");
            ...
            $status_label = array(
                'js_lang_label.REQUEST_MANAGEMENT_ACCEPT',
                'js_lang_label.REQUEST_MANAGEMENT_REJECT',
                'js_lang_label.REQUEST_MANAGEMENT_PENDING',
            );

            $list_config = $this->request_management_model->getListConfiguration();
            $this->processConfiguration($list_config, $add_access, $edit_access, TRUE);
            $this->general->trackModuleNavigation("Module", "List", "Viewed", $this->mod_enc_url["index"], "request_management");
            ...
            $render_arr = array(
                ...
                'default_filters' => $this->request_management_model->default_filters,
            );
            ...
            {
                $this->loadView("request_management_index");
            }
        }
        catch(Exception $e)
        {
    ...
    public function listing()
    {
        ...
        $list_config = $this->request_management_model->getListConfiguration();
        $form_config = $this->request_management_model->getFormConfiguration();
        $extra_cond = $this->request_management_model->extra_cond;
        $groupby_cond = $this->request_management_model->groupby_cond;
        $having_cond = $this->request_management_model->having_cond;
        $orderby_cond = $this->request_management_model->orderby_cond;
        ...
        //pr($data_config);die
        $data_recs = $this->request_management_model->getListingData($data_config);

        //print_r($data_recs); exit;
        $data_recs['no_records_msg'] = $this->general->processMessageLabel('ACTION_NO_REQUEST_MANAGEMENT_DATA_FOUND_C46_C46_C33');
    ...
    public function export()
    {
        if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
        {
            $this->filter->checkAccessCapability("request_management_export");
        }
        else
        {
            $this->filter->getModuleWiseAccess("request_management", "Export", TRUE);
        }
        $params_arr = $this->params_arr;
        $page = $params_arr['page'];
        ...
        $list_config = $this->request_management_model->getListConfiguration();
        $form_config = $this->request_management_model->getFormConfiguration();
        $table_name = $this->request_management_model->table_name;
        $table_alias = $this->request_management_modeltable_alias;
        $primary_key = $this->request_management_model->primary_key;
        $extra_cond = $this->request_management_model->extra_cond;
        $groupby_cond = $this->request_management_model->groupby_cond;
        $having_cond = $this->request_management_model->having_cond;
        $orderby_cond = $this->request_management_model->orderby_cond;
        ...
        $db_recs = $this->request_management_model->getExportData($export_config);
        ...
        $misc_info['heading'] = $this->lang->line('REQUEST_MANAGEMENT_REQUEST_MANAGEMENT');
        $misc_info['filename'] = "request_management_records_".count($db_recs);
        ...
        if ($export_type == 'pdf')
        {
            ...
            {
                $pdf->setModule("request_management_model");
            }
        ...
        try
        {
            ...
            if ($mode == "Update")
            {
                ...
                else
                {
                    ...
                    list($list_access, $view_access, $edit_access, $del_access, $print_access) = $this->filter->getModuleWiseAccess("request_management", $access_list, TRUE, TRUE);
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    list($list_access, $add_access) = $this->filter->getModuleWiseAccess("request_management", $access_list, TRUE, TRUE);
                }
            ...
            if ($mode == 'Update')
            {
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowEdit", "request_management"), $this->session->userdata('iAdminId'));
                $data_arr = $this->request_management_model->getData(intval($id));
                ...
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "request_management", $recName);
            }
            else
            {
                $recName = '';
                $ctrl_flow = $this->ci_local->read($this->general->getMD5EncryptString("FlowAdd", "request_management"), $this->session->userdata('iAdminId'));
                $this->general->trackModuleNavigation("Module", "Form", "Viewed", $this->mod_enc_url["add"], "request_management");
            }

            $opt_arr = $img_html = $auto_arr = $config_arr = array();

            $form_config = $this->request_management_model->getFormConfiguration($config_arr);
                ...
                foreach ($form_config as $key => $val)
                {
                    ...
                    if ($val['function'] != "")
                    {
                        ...
                        elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                        {
                            ...
                            if (method_exists($this->request_management_model, $phpfunc))
                            {
                                $tmpdata = $this->request_management_model->$phpfunc($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                            }
                        }
                    ...
                    if ($val['field_status'] != "")
                    {
                        ...
                        elseif ($status_type == "function")
                        {
                            ...
                            elseif (substr($fd_callback, 0, 7) == 'model::' && substr($fd_callback, 7) !== FALSE)
                            {
                                ...
                                if (method_exists($this->request_management_model, $fd_callback))
                                {
                                    $fd_status = $this->request_management_model->$fd_callback($mode, $data[$key], $data, $id, $key, $key, $this->module_name);
                                }
                            }
            ...
            else
            {
                if ($mode == "Update")
                {
                    ...
                    {
                        $this->loadView("request_management_add");
                    }
                    else
                    {
                        $this->loadView("request_management_add_view");
                    }
                }
                else
                {
    ...
    public function addAction()
    {
        ...
        try
        {
            ...
            if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
            {
                if ($mode == "Update")
                {
                    $add_edit_access = $this->filter->checkAccessCapability("request_management_update", TRUE);
                }
                else
                {
                    $add_edit_access = $this->filter->checkAccessCapability("request_management_add", TRUE);
                }
            }
            else
            {
                $add_edit_access = $this->filter->getModuleWiseAccess("request_management", $mode, TRUE, TRUE);
            }
            if (!$add_edit_access)
            {
                if ($mode == "Update")
            ...
            if ($mode == 'Add')
            {
                ...
                $id = $this->request_management_model->insert($data);
                if (intval($id) > 0)
                {
                    $save_data_arr["iUser_request_id"] = $data["iUser_request_id"] = $id;
                ...
                $track_cond = $this->db->protect("iUser_request_id")." = ".$this->db->escape($id);
                $switch_combo = $this->request_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Added", $this->mod_enc_url["add"], "request_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            elseif ($mode == 'Update' && false== $strCheck)
            {
                ...
                $res = $this->request_management_model->update($data, intval($id));
                if (intval($res) > 0)
                {
                    $save_data_arr["iUser_request_id"] = $data["iUser_request_id"] = $id;
                ...
                $switch_combo = $this->request_management_model->getSwitchTo($track_cond);
                $recName = $switch_combo[0]["val"];
                $this->general->trackModuleNavigation("Module", "Form", "Modified", $this->mod_enc_url["add"], "request_management", $recName, "mode|".$this->general->getAdminEncodeURL("Update")."|id|".$this->general->getAdminEncodeURL($id));
            }
            $ret_arr['id'] = $id;
            $ret_arr['mode'] = $mode;
             if(true==$strCheck)
            {
                $msg = "This request is already in used. So can not be Deleted or Inactivated";
            }else{
                $ret_arr['message'] = $msg; 
            }
            $ret_arr['message'] = $msg;
            ...
            /*$file_data["vrequest_images"]["file_name"] = $request_images;
            $file_data["vrequest_images"]["old_file_name"] = $params_arr["vrequest_images"];
            $file_data["vrequest_images"]["unique_name"] = "vrequest_images";
            $file_data["vrequest_images"]["primary_key"] = "iUser_request_id";*/
    ...
    public function inlineEditAction()
    {
     ...
     //  echo "str--".$strCheck; exit;
        if ($all_row_selected == "true" && in_array($operartor, array("del", "status")) && false==$strCheck)
        {
            ...
            $config_arr['list_config'] = $this->request_management_model->getListConfiguration();
            $config_arr['form_config'] = $this->request_management_model->getFormConfiguration();
            $config_arr['table_name'] = $this->request_management_model->table_name;
            $config_arr['table_alias'] = $this->request_management_model->table_alias;
        ...
        if ($search_alias == "Yes")
        {
            $primary_field = $this->request_management_model->table_alias.".".$this->request_management_model->primary_key;
        }
        else
        {
            $primary_field = $this->request_management_model->primary_key;
        }
        ...
        try
        {
            ...
            switch ($operartor)
            {
                case 'del':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $del_access = $this->filter->checkAccessCapability("request_management_delete", TRUE);
                    }
                    else
                    {
                        $del_access = $this->filter->getModuleWiseAccess("request_management", "Delete", TRUE, TRUE);
                    }
                    if (!$del_access)
                    {
                    ...
                    if (is_array($primary_ids))
                    {
                        $main_cond = $this->db->protect("iUser_request_id")." IN ('".implode("','", $primary_ids)."')";
                    }
                    else
                    {
                        $main_cond = $this->db->protect("iUser_request_id")." = ".$this->db->escape($primary_ids);
                    }
                    if(false==$strCheck)
                    {
                       $success = $this->request_management_model->delete($extra_cond, $search_alias, $search_join);  
                    }else{
                        $success = 1;
                    }
                    $main_data = $this->request_management_model->getData($main_cond, array());
                    $success = $this->request_management_model->delete($extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_FAILURE_IN_DELETION_THIS_RECORD_C46_C46_C33'));
                ...
                case 'edit':
                    ...
                    if ($this->config->item("ENABLE_ROLES_CAPABILITIES"))
                    {
                        $edit_access = $this->filter->checkAccessCapability("request_management_update", TRUE);
                    }
                    else
                    {
                        $edit_access = $this->filter->getModuleWiseAccess("request_management", "Update", TRUE, TRUE);
                    }
                    if (!$edit_access)
                    {
                        throw new Exception($this->general->processMessageLabel('ACTION_YOU_ARE_NOT_AUTHORIZED_TO_MODIFY_THESE_DETAILS_C46_C46_C33'));
                    ...
                    $list_config = $this->request_management_model->getListConfiguration($post_name);
                    $form_config = $this->request_management_model->getFormConfiguration($list_config['source_field']);
                    if (!is_array($form_config) || count($form_config) == 0)
                    {
                    ...
                    $db_rec_arr = $this->request_management_model->getData(intval($primary_ids));

  
                    $data_arr[$field_name] = $post_val;
                    if(false==$strCheck)
                    {
                      $success = $this->request_management_model->update($data_arr, intval($primary_ids)); 
                    }else{
                        $success = 1;
                    }
                    $success = $this->request_management_model->update($data_arr, intval($primary_ids));
                ...
                case 'status':
                    ...
                    {
                        $field_name = $this->request_management_model->table_alias.".eStatus";
                    }
                    else
                    {
                    ...
                    if(false==$strCheck)
                    {
                      $success = $this->request_management_model->update($data_arr, $primary_ids);
                    }else{
                        $success = 1;                       
                    }
                    //$success = $this->request_management_model->update($data_arr, $extra_cond, $search_alias, $search_join);
                    if (!$success)
                    {
                     ...
                     if(false==$strCheck)
                     {
                        ...
                        $message = "This request is already in used. So can not be Deleted or Inactivated"; 
                     }
            ...
            if ($mode == "Update")
            {
                ...
                if ($form_config["type"] == "file")
                {
                    ...
                    $file_data[$unique_name]["primary_key"] = "iUser_request_id";
    ...
    public function getSourceOptions($name = '', $mode = 'Add', $id = '', $data = array(), $extra = '', $rtype = 'records')
    {
        ...
        switch ($type)
        {
            ...
            case 'table':
                ...
                {
                    $combo_config['main_table'] = $this->request_management_model->table_name;
            ...
            case 'phpfn':
                ...
                elseif (substr($phpfunc, 0, 7) == 'model::' && substr($phpfunc, 7) !== FALSE)
                {
                    ...
                    if (method_exists($this->request_management_model, $phpfunc))
                    {
                        $data_arr = $this->request_management_model->$phpfunc($data[$name], $mode, $id, $data, $parent_src, $this->term);
                    }
                }
                elseif (method_exists($this->general, $phpfunc))
                {
    ...
    public function getSelfSwitchTo()
    {
        ...
        $switchto_fields = $this->request_management_model->switchto_fields;
        $extra_cond = $this->request_management_model->extra_cond;
        ...
        $switch_arr = $this->request_management_model->getSwitchTo($extra_cond);
    ...
    public function getListOptions()
    {
        ...
        $config_arr = $this->request_management_model->getListConfiguration($alias_name);
    ...
    public function downloadListFile()
    {
        ...
        if (is_array($config_arr) && count($config_arr) > 0)
        {
            ...
            $data_arr = $this->request_management_model->getData(intval($id), $fields, '', '', '', 'Yes');
    ...
    public function uploadFormFile()
    {
        ...
        $config_arr = $this->request_management_model->getFormConfiguration($unique_name);
    ...
    public function deleteFormFile()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                $res = $this->request_management_model->update($update_arr, intval($id));
                if (!$res)
                {
