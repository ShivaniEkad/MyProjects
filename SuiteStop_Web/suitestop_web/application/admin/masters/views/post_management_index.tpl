...
<%javascript%>
    ...
    },
    {
     ...
     {
        "name": "post_details",
        "label": "<%$list_config['post_details']['label_lang']%>"
    },
    {
        "name": "created_at",
    ...
    js_col_model_json = [
    {
        ...
        "editrules": {
            ...
            "infoArr": {
                "required": {
                    "message": ci_js_validation_message(js_lang_label.GENERIC_PLEASE_ENTER_A_VALUE_FOR_THE__C35FIELD_C35_FIELD_C46 ,"#FIELD#",js_lang_label.POST_MANAGEMENT_USER_NAME)
                }
            }
        },
        "searchoptions": {
            "attr": {
                ...
                "aria-module-name": "post_management",
    ...
    }, 
    {
        ...
        "editrules": {
            ...
            "infoArr": {
                "required": {
                    "message": ci_js_validation_message(js_lang_label.GENERIC_PLEASE_ENTER_A_VALUE_FOR_THE__C35FIELD_C35_FIELD_C46 ,"#FIELD#",js_lang_label.POST_MANAGEMENT_NAME)
                }
            }
        },
        "searchoptions": {
    ...
    }, 
    {
        ...
        "labelClass": "header-align-left",
        "resizable": true,
        "width": "<%$list_config['post_details']['width']%>",
        "search": <%if $list_config['post_details']['search'] eq 'No' %>false<%else%>true<%/if%>,
        "export": <%if $list_config['post_details']['export'] eq 'No' %>false<%else%>true<%/if%>,
        "sortable": <%if $list_config['post_details']['sortable'] eq 'No' %>false<%else%>true<%/if%>,
        "hidden": <%if $list_config['post_details']['hidden'] eq 'Yes' %>true<%else%>false<%/if%>,
        "hideme": <%if $list_config['post_details']['hideme'] eq 'Yes' %>true<%else%>false<%/if%>,
        "addable": <%if $list_config['post_details']['addable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "editable": <%if $list_config['post_details']['editable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "align": "left",
        "edittype": "text",
        "editrules": {
            "required": true,
            "infoArr": {
                "required": {
                    "message": ci_js_validation_message(js_lang_label.GENERIC_PLEASE_ENTER_A_VALUE_FOR_THE__C35FIELD_C35_FIELD_C46 ,"#FIELD#",js_lang_label.POST_MANAGEMENT_DETAILS)
                }
            }
        },
        "searchoptions": {
            "attr": {
                "aria-grid-id": el_tpl_settings.main_grid_id,
                "aria-module-name": "post_management",
                "aria-unique-name": "post_details",
                "autocomplete": "off"
            },
            "sopt": strSearchOpts,
            "searchhidden": <%if $list_config['post_details']['search'] eq 'Yes' %>true<%else%>false<%/if%>
        },
        "editoptions": {
            "aria-grid-id": el_tpl_settings.main_grid_id,
            "aria-module-name": "post_management",
        ...
        "ctrl_type": "textbox",
        "default_value": "<%$list_config['post_details']['default']%>",
        "filterSopt": "bw",
        "formatter": formatAdminModuleEditLink,
        "unformat": unformatAdminModuleEditLink
    },
