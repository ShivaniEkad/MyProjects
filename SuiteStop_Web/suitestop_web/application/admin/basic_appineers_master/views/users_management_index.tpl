...
<%javascript%>
    ...
    {
        "name": "u_usertype",
        "label": "<%$list_config['u_usertype']['label_lang']%>"
    },
    {
        "name": "u_updated_at",
    ...
    },
    {
        ...
        "labelClass": "header-align-center",
        "resizable": true,
        "width": "<%$list_config['u_usertype']['width']%>",
        "search": <%if $list_config['u_usertype']['search'] eq 'No' %>false<%else%>true<%/if%>,
        "export": <%if $list_config['u_usertype']['export'] eq 'No' %>false<%else%>true<%/if%>,
        "sortable": <%if $list_config['u_usertype']['sortable'] eq 'No' %>false<%else%>true<%/if%>,
        "hidden": <%if $list_config['u_usertype']['hidden'] eq 'Yes' %>true<%else%>false<%/if%>,
        "hideme": <%if $list_config['u_usertype']['hideme'] eq 'Yes' %>true<%else%>false<%/if%>,
        "addable": <%if $list_config['u_usertype']['addable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "editable": <%if $list_config['u_usertype']['editable'] eq 'Yes' %>true<%else%>false<%/if%>,
        "align": "center",
        "edittype": "select",
        "editrules": {
            "required": true,
            "infoArr": {
                "required": {
                    "message": ci_js_validation_message(js_lang_label.GENERIC_PLEASE_ENTER_A_VALUE_FOR_THE__C35FIELD_C35_FIELD_C46 ,"#FIELD#",js_lang_label.USERS_MANAGEMENT_USER_TYPE)
                }
            }
        },
        "searchoptions": {
            ...
            "sopt": intSearchOpts,
            "searchhidden": <%if $list_config['u_usertype']['search'] eq 'Yes' %>true<%else%>false<%/if%>,
            "dataUrl": <%if $count_arr["u_usertype"]["json"] eq "Yes" %>false<%else%>'<%$admin_url%><%$mod_enc_url["get_list_options"]%>?alias_name=u_usertype&mode=<%$mod_enc_mode["Search"]%>&rformat=html<%$extra_qstr%>'<%/if%>,
            "value": <%if $count_arr["u_usertype"]["json"] eq "Yes" %>$.parseJSON('<%$count_arr["u_usertype"]["data"]|@addslashes%>')<%else%>null<%/if%>,
            "dataInit": <%if $count_arr['u_usertype']['ajax'] eq 'Yes' %>initSearchGridAjaxChosenEvent<%else%>initGridChosenEvent<%/if%>,
            "ajaxCall": '<%if $count_arr["u_usertype"]["ajax"] eq "Yes" %>ajax-call<%/if%>',
            "multiple": true
        },
        "editoptions": {
            "aria-grid-id": el_tpl_settings.main_grid_id,
            "aria-module-name": "users_management",
            "aria-unique-name": "u_usertype",
            "dataUrl": '<%$admin_url%><%$mod_enc_url["get_list_options"]%>?alias_name=u_usertype&mode=<%$mod_enc_mode["Update"]%>&rformat=html<%$extra_qstr%>',
            "dataInit": <%if $count_arr['u_usertype']['ajax'] eq 'Yes' %>initEditGridAjaxChosenEvent<%else%>initGridChosenEvent<%/if%>,
            "ajaxCall": '<%if $count_arr["u_usertype"] eq "Yes" %>ajax-call<%/if%>',
            "data-placeholder": "<%$this->general->parseLabelMessage('GENERIC_PLEASE_SELECT__C35FIELD_C35' ,'#FIELD#', 'USERS_MANAGEMENT_USER_TYPE')%>",
            "class": "inline-edit-row chosen-select"
        },
        "ctrl_type": "dropdown",
        "default_value": "<%$list_config['u_usertype']['default']%>",
        "filterSopt": "in",
        "stype": "select"
    },
    {
