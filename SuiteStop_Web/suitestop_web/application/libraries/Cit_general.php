...
class Cit_general extends General
{
    ...
    public function verifyUserType($input_params = array())
    {
        $old_user_type = $input_params['old_user_type'];

        $existing_ut = $input_params['mc_user_type'];      

        if ($old_user_type == $existing_ut) {
            $return_arr[0]['is_matched'] = 1;
        } else {
            $return_arr[0]['is_matched'] = 0;
        }
    ...
    public function encryptUserType($value = '', $data_arr = array())
    {
        $user_type = $value;

        $encrypt_type = "bcrypt";

        $encrypt_user_type = $this->CI->general->encryptDataMethod($user_type, $encrypt_type);
            
        return $encrypt_user_type;
    }
